...
def _render_site_performance(sd: dict, is_primary: bool = True) -> None:
    ...
    perf_data = scores.get("performance_technical_health", {})
    ...
    if mobile_lh or desktop_lh:
        ...
        for i, cat_key in enumerate(PERFORMANCE_CATEGORIES):
            with cols[i]:
                ...
                # Show gauge for the primary (mobile) score
                ...
                if primary_val is not None:
                    ...
                    # Show gap indicator if significant
    ...
    if cwv_data:
        ...
        for i, key in enumerate(row1_keys):
            with cols1[i]:
                ...
                st.markdown(cwv_metric_html(key, val), unsafe_allow_html=True)

        # Second row: TBT, Speed Index, INP
        ...
        # Show desktop comparison if gaps are significant
    ...
    _render_tech_stack(bw)
    st.markdown("---")

    # === Critical Audit Failures — Grouped by Category ===
    _render_audit_failures(lh)
...
def _render_tech_stack(bw: dict) -> None:
    ...
    for cat_lower in priority_cats:
        for cat_name in list(grouped.keys()):
            if cat_name.lower() == cat_lower:
                _render_tech_category(cat_name, grouped[cat_name])
...
def _render_tech_category(category: str, techs: list[dict]) -> None:
    ...
    for i, t in enumerate(techs):
        with cols[i % 3]:
            ...
            st.markdown(
                tech_card_html(name, desc, category),
                unsafe_allow_html=True,
            )


def _render_audit_failures(lh: dict) -> None:
    ...
    # Filter to failing audits with weight > 0
    failures = [
        a for a in audits
